...
export class OrderForm extends TemplateComponent implements IOrderForm {
		...
		updateFromModel(): void {
				if (this._orderModel) {
						...
						this._addressInput.value = this._orderModel.getAddress();

						// Обновляем выбранный способ оплаты
						const paymentMethod = this._orderModel.getPaymentMethod();
						if (paymentMethod) {
								this._paymentButtons.forEach((button) => {
										if (
												(paymentMethod === 'online' && button.name === 'card') ||
												(paymentMethod === 'cash' && button.name === 'cash')
										) {
												button.classList.add(CLASS_NAMES.BUTTON_SELECTED);
										} else {
												button.classList.remove(CLASS_NAMES.BUTTON_SELECTED);
								...
								// Сбрасываем выделение всех кнопок, если метод оплаты не выбран
								this._paymentButtons.forEach((button) => {
										button.classList.remove(CLASS_NAMES.BUTTON_SELECTED);
								});
						}
